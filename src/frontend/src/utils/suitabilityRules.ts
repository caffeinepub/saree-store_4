...
function getBodyTypeFabricScore(
  bodyType: BodyType,
  fabrics: string[],
  styles: string[],
): number {
...
function generateRecommendation(
  ...
  const colorStr =
    colors.length > 0 ? colors.slice(0, 2).join(" and ") : "vibrant";
...
function generateTips(
  ...
  const bodyTips: Record<BodyType, string> = {
    petite:
      "Lightweight fabrics and vertical patterns create an elongating effect.",
    average:
      "You can carry most styles — experiment with bold prints and heavy embroidery.",
  ...
  const occasionTips: Record<Occasion, string> = {
    casual:
      "Pair with simple jewelry and comfortable footwear for an effortless look.",
    wedding:
      "Heavy gold jewelry and a statement blouse will complete the bridal look.",
    festive:
      "Opt for statement earrings and a contrasting blouse for a festive vibe.",
    formal:
      "Keep accessories minimal and choose a well-fitted blouse for a polished look.",
  };
...
export function calculateSuitability(
  ...
  const weightedScore = Math.round(
    colorScore * 0.4 + bodyScore * 0.3 + occasionScore * 0.3,
  );
